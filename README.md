...
- [Local Backends On Mac](#local-backends-on-mac)
- [Integrated Runtime Management](#integrated-runtime-management)
- [Model Configuration On Mac](#model-configuration-on-mac)
- [Local Model Recommendation For This Mac](#local-model-recommendation-for-this-mac)
- [Local Agent Scripts](#local-agent-scripts)
- [Mac Desktop Shell](#mac-desktop-shell)
- [Desktop Web Shell](#desktop-web-shell)
- [iPhone Web Shell](#iphone-web-shell)
...
- Node.js `18.18+`
- 已安装并能运行 `codex`
- macOS: 已授予终端或启动进程 `Screen Recording` 权限
...
- 启动脚本默认会打开本机桌面控制台：`/mac`

### Windows

当前已经接入 Windows 抓屏 backend，本机运行 agent 时会自动选择 `windows`。

Windows 上优先使用脚本启动和停止：
...
- 当前 `scripts/dev/*.command` 和 `Mac 原生壳` 都是 macOS 专用
- Windows 上优先使用 `scripts/dev/*.ps1`
- 抓屏 backend 默认自动选择 `powershell.exe + scripts/windows/capture-screen.ps1`
...
- `SCREEN_PILOT_CAPTURE_BACKEND`: 抓屏 backend，支持 `auto`、`macos`、`windows`，默认 `auto`
- `CAPTURE_BIN`: 抓屏命令路径；macOS 默认 `/usr/sbin/screencapture`，Windows 默认 `powershell.exe`
...
- 抓屏 backend 默认按运行平台自动选择：`darwin -> macos`，`win32 -> windows`
- Windows backend 当前通过 `powershell.exe` 调 [scripts/windows/capture-screen.ps1](/Users/oyzh/interview/scripts/windows/capture-screen.ps1) 抓主显示器整屏
